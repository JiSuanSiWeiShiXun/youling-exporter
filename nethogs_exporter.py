...
# global f_opr
# f_opr = open('/opt/node_exporter/custom_data/net.prom', mode='w')
...
LIBRARY_NAME = 'lib/libnethogs.so.'
...
FILTER = 'udp portrange 4800-4900'
...
def network_activity_callback(action, data):
    # if "unknown" in str(data.contents.name):
    #     return
...
#     global group_names
#     global net_stats
#     if data.contents.name in group_names:
#         global f_opr
#         f_opr.seek(0)
#         f_opr.truncate()
#         f_opr.writelines(net_stats)
#         f_opr.flush()
#         net_stats = []
#         group_names = {}
#     else:
#         net_stats.append('nethogs_process_net_io_sent_bytes_total{groupname="%s"} %s\n' % (data.contents.name, data.contents.sent_bytes))
#         net_stats.append('nethogs_process_net_io_recv_bytes_total{groupname="%s"} %s\n' % (data.contents.name, data.contents.recv_bytes))
#         group_names[data.contents.name] = 1
