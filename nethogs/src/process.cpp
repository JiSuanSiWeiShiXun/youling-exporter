...
/*
* returns the process from UDP port
* modifier : wsj
*/
Process* getUDPProcess(short int packettype, const char* hashstring) {
    if (packettype == IPPROTO_UDP)
    {
        //std::cout << "No PID information UDP getProcess " << std::endl;
        Process* proc = findProcessByName(hashstring);
        if (proc != NULL)
            return proc;

        Process* newproc = new Process(0, "", hashstring);
  ...
  if (inode != 0) {
    ...
    if (packettype == IPPROTO_TCP) {
      proc = unknowntcp;
        ...
        proc = getUDPProcess(IPPROTO_UDP, strtok(connection->refpacket->gethashstring(),"-"));
      //proc = unknownudp;
    }
  }
